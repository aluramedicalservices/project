<template>
  <div id="nav-top" class="text-noxgrey fixed top-0 left-0 w-full z-50">
    <div id="nav-menu" class="bg-healingblue flex flex-row justify-between py-4 px-6">
      <div id="icono-menu-hamburguesa" class="text-noxgrey" @click="toggleMenu">
        <Menu />
      </div>
      <div @click="irANotificaciones" id="icono-notificaciones" class="text-noxgrey">
        <Bell />
      </div>
    </div>
...
<script>
import { Bell, Menu, House, Calendar, Cross, History, CreditCard, User, Users, Map, Settings, X } from 'lucide-vue-next';
...
export default {
  ...
  methods: {
    ...
    irANotificaciones() {
      this.$router.push('/notificaciones-medicas');  // ← Aquí navegas a la vista de notificaciones
    }
  }
}
</script>
